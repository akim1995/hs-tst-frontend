  ...
  (let [navigate (useNavigate)
        on-submit-hdlr (fn [e]
                         ...
                         (let [form-data-obj (js/FormData. (.-target e))
                               patient (into {} (for [field ["name" "gender" "date-of-birth" "health-insurance-number" "address"]]
                                                  [(keyword field) (.get form-data-obj field)]))]
                           (.then (patient-http-client/post-data (update patient :date-of-birth (fn [dob] (.toISOString (js/Date. dob))))) #(navigate "/"))))]

    (d/div {:class "w-110 mx-auto px-4 py-4 shadow-sm border border-gray-200 mt-8 rounded-lg"}
           ...
           (d/form {:on-submit on-submit-hdlr :class "space-y-4"}
                   ...
                   (d/div {:class "flex justify-end w-96 space-x-2"}
                          (d/button {:on-click #(navigate "/") :type "button" :class "cursor-pointer px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-md shadow-sm hover:bg-gray-700"} "Cancel")
